...
model = ExplainableModel(Config.model_name, 1).to(Config.device)

train_dataset = MyDataset('dataset/train.txt', Config.max_length)
validate_dataset = MyDataset('dataset/dev.txt', Config.max_length)
test_dataset = MyDataset('dataset/test.txt', Config.max_length)
...
for epoch in range(Config.epochs):
    ...
    for step, batch in bar:
        ...
        train_acc = pl.metrics.Accuracy()
        valid_acc = pl.metrics.Accuracy()

        acc = train_acc(predict_labels, label)
    ...
    for step, batch in bar:
        ...
        acc = valid_acc(predict_labels, label)
